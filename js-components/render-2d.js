...
function setup() {
  var canvas = createCanvas(D.cw, D.ch);
...
function draw() {
  ...
  background(30, 30, 50);
  drawLatent(t);
  stroke(255, 255, 255, 12); strokeWeight(1); line(0, D.lh, D.cw, D.lh);
  drawWaveform(t);
  stroke(255, 255, 255, 12); strokeWeight(1); line(0, D.lh + D.wh, D.cw, D.lh + D.wh);
  drawCentroid(t);
  ...
  stroke(255, 0, 0); strokeWeight(2); noFill(); rect(0, 0, D.cw - 1, D.ch - 1);
}
...
function drawLatent(t) {
  ...
  for (var i = 0; i <= drawN && i < n; i++) {
    ...
    circle(pts[i][0], pts[i][1], 4);
  }
  var idx = Math.min(drawN, n - 1);
  var cx = pts[idx][0];
  var cy = pts[idx][1];
  noStroke();
  for (var r = 24; r > 0; r -= 4) {
    fill(255, 215, 0, 200 - r * 8);
    circle(cx, cy, r);
  }
  fill(255, 215, 0);
  circle(cx, cy, 5);
...
function drawWaveform(t) {
  ...
  fill(0, 210, 255, 18);
  beginShape();
  for (var i = 0; i + 1 < ws.length; i += 2) {
    vertex(ws[i] * w, ws[i + 1] * h);
  }
  endShape(CLOSE);
  if (prog > 0.02) {
    noStroke();
...
function drawCentroid(t) {
  ...
  if (idx >= 0 && idx < n) {
    ...
    var cy = pts[idx][1];
    noStroke();
    for (var r = 16; r > 0; r -= 3) {
      fill(255, 215, 0, 140 - r * 8);
      circle(cx, cy, r);
    }
    fill(255, 215, 0);
    circle(cx, cy, 5);
